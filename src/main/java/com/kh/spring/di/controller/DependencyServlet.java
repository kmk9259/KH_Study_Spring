...
public class DependencyServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * */
		//1. 일반 클래스 생성 - 버전1에서 버전 2로 변경 시 모든 내용을 변경해야하는 경우가 생김, 이를 결합도가 높다고 표현 -> 유지보수에 어려움
		//  		    - 개발자가 new를 통해 객체 직접 생성
		...
		//2. 인터페이스를 사용하여 결합도 낮추기 - 개발자가 new를 통해 객체를 직접 생성
		/*Singer singer = new BlackPink();
		singer.compose();
		singer.sing();
		singer.dance();*/
		
		//3. 전략 디자인 패턴을 이용하여 결합도 낮추기 - 전달 받은 값에 따라 해당하는 객체를 바꿔서 만들어주는 방식
		...
		//네이밍 할 때 큰 기능으로 네이밍해서 중복이 없게끔 해라. 회사에서 네이밍 명명규칙 물어볼것
		singer.compose();
		singer.sing();
		singer.dance();
